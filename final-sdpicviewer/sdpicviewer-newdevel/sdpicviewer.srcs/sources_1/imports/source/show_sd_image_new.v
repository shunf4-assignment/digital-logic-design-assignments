...
/* ------------ SD AND SPI CONTROLLER INSTANTIATION SECTION ------------ */
    ...
    wire [31:0] curr_vga_x;
    wire [31:0] curr_vga_y;
    //720x400
...
//BTNL, BTNR 捕获器
    ...
    assign curr_vga_y = (whratio_p1 >= whratio_p2) ? (
                            (curr_bmp_height[31] == 0) ? ((VGA_H - VGA_W * curr_bmp_height_abs / curr_bmp_width) / 2 + (curr_bmp_height_abs - curr_bmp_y - 1) * VGA_W / curr_bmp_width) : ((VGA_H - VGA_W * curr_bmp_height_abs / curr_bmp_width) / 2 + curr_bmp_y * VGA_W / curr_bmp_width)
                        ) : (
                            (curr_bmp_height[31] == 0) ? (curr_vga_y <= (curr_bmp_height_abs - curr_bmp_y - 1) * VGA_H / curr_bmp_height_abs) : (curr_vga_y <= curr_bmp_y * VGA_H / curr_bmp_height_abs)
                        );

    assign curr_vga_x = (whratio_p1 >= whratio_p2) ? (
                            curr_bmp_x * VGA_W / curr_bmp_width
                        ) : (
                            (VGA_W - VGA_H * curr_bmp_width / curr_bmp_height_abs) / 2 + curr_bmp_x * VGA_H / curr_bmp_height_abs
                        );
