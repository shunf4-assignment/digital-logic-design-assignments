...
/* ------------ SEQUENCE TIMING CONTROLLER SECTION ------------ */
    ...
    parameter S_READBMPHEADER = 8'h31;
    parameter S_READBMPPALETTE = 8'h32;
    parameter S_READBMPPALETTE_DO = 8'h33;
    parameter S_READBMP = 8'h34;
    ...
    assign curr_bmp_real_y = (curr_bmp_height >= 0) ? (curr_bmp_height_abs - y - 1) : curr_bmp_y;
        ...
        if(rst) begin
            ...
            case(main_state)
                ...
                begin
                    ...
                    if(
                        {{root_cluster[diritem_index << 5 + 'd20] , root_cluster[diritem_index << 5 + 'd21], root_cluster[diritem_index << 5 + 'd26], root_cluster[diritem_index << 5 + 'd27]} != 32'h0 && //簇号不为0
                        
                        {root_cluster[diritem_index << 5 + 'd28],root_cluster[diritem_index << 5 + 'd29],root_cluster[diritem_index << 5 + 'd30],root_cluster[diritem_index << 5 + 'd31]} > 0 && {root_cluster[diritem_index << 5 + 'd28],root_cluster[diritem_index << 5 + 'd29],root_cluster[diritem_index << 5 + 'd2A],root_cluster[diritem_index << 5 + 'd2B]} < (1 << 20) && //文件大小在1M以内

                        (root_cluster[diritem_index << 5 + 11] & 8'b00001111) != 8'b00001111 && //不具有 LONG_NAME 属性

                        {root_cluster[diritem_index << 5 + 8],root_cluster[diritem_index << 5 + 9],root_cluster[diritem_index << 5 + 10]} == 24'h424d50 //扩展名为BMP
                        ) begin
                ...
                begin
                    result_fat_nextclus <= root_fat_sector[(query_fat_clus << 2) & 9'b111111111 +: 4];
                ...
                begin
                    if({bmp_cluster[0], bmp_cluster[1]} != 16'h424D)begin
                    ...
                    end else if({bmp_cluster['h19],bmp_cluster['h18],bmp_cluster['h17],bmp_cluster['h16]}[31] == 1'b1) begin
                        //正向位图
                    end else if({bmp_cluster['h21],bmp_cluster['h20],bmp_cluster['h1f],bmp_cluster['h1e]} != 0) begin
                        //压缩位图
                    end else if({bmp_cluster['h31],bmp_cluster['h30],bmp_cluster['h2f],bmp_cluster['h2e]} != 0) begin
                        //有不同于默认值调色板的位图
                    end else if({bmp_cluster['h1d],bmp_cluster['h1c]} != 24) begin
                        //不支持的位深
                    end
                    else begin
                        ...
                        curr_bmp_bibitcount <= {bmp_cluster['h1d],bmp_cluster['h1c]};
